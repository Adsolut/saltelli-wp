...
**Last updated:** 2026-05-13 (Wave 6.0 partial STABILIZZATA dopo 2 incident WSOD + sanitize chirurgico. Storia: (1) v1.3.22 merge Wave 6.0 partial — template patch `apply_filters('the_content', $body_ext)` + script migration ready. (2) Wet-run script su staging: 16 CPT competenza MIGRATED (post_content → body_extended postmeta), 3 tier-1 SKIP already-populated, 5 draft SKIP not-publish. Backup `_legacy_post_content_backup` postmeta integro per ogni post migrato. (3) WSOD frontend rilevato post-deploy → causa: `apply_filters('the_content', ...)` su content legacy migrato → fatal. **Rollback template v1.3.23** (`echo wp_kses_post($body_ext)`) ripristina frontend. (4) WSOD admin edit residuo solo post 2670 (e simili migrati): admin metabox SCF wysiwyg TinyMCE crash su raw HTML legacy compilato 2019-2020. **Sanitize chirurgico applicato via WP-CLI DB ops** su staging: per i 16 migrated → strip control chars (`\\x00-\\x08\\x0B\\x0C\\x0E-\\x1F\\x7F`) + normalize line endings + `wp_kses_post()` sanitize (strip script/iframe/handlers, preserva markup semantico). Tutte le 19 competenze ora admin-editable + frontend invariato. **Wave 6.0 partial chiusa**. Wave 6.0 full (disable Gutenberg per CPT competenza) resta backlog post-cut.)
**Branch:** `main` · last merge `d93b50a` (rollback template v1.3.23) · prev merge `9b5221c` (Wave 6.0 partial v1.3.22) · sanitize DB ops eseguito post-rollback (no commit codice, solo DB staging) · ~46 commit totali post-v1.3.13
**Demo:** ✅ presentata al cliente · feedback iteration assorbita · feedback Elena 23 punti triage completato (13 chiusi Batch 1 + 3 chiusi Batch 2 + 1 chiuso Batch 3 + 1 chiuso Wave 6.0 partial #23 v2 = 18 chiusi · 1 già risolto pre-batch · 1 audit deferred zero-code (#15 = azione editoriale Elena) · 3 defer post-cut)
**Live staging:** https://staging.studiolegalesaltelli.it allineato v1.3.23 template rollback (sanitize DB ops applicato post-deploy) · **13 Page WP Gutenberg-disabled** (12 + Prenota appuntamento 2714) · **18 Pages canoniche** · **Design Handoff completato 12 template** · **Wave Elena FB Batch 1+2+3 + Wave 6.0 partial COMPLETATI**: 18 fix Elena-approved totali · tokens.css rebuilt da DESIGN.md (Wave 5 STEP 2) · sections.css drift cleanup STEP 4 (328 token swaps + Design Handoff ~15 fix CSS) · cross-page smoke test PASS
**Active phase:** Elena pre-cut minor UX fix via WP-Admin (popolare scenari term contenzioso/imprese + content rifinitura libera) · cleanup 15 branch feat/chore orfani (post-cut housekeeping) · attesa cliente per finestra cut produzione (DNS switch staging → prod)
**Next:** Cut produzione (DNS switch staging → prod) · backlog post-cut: **Wave 6.0 full** (disable Gutenberg per CPT competenza — chiude tech debt sanitize manuale future + previene re-pollution post_content) · **Wave 6.1 SCF orphan cleanup** (13 field documentati `docs/SCF_ORPHAN_FIELDS.md` + script migration `prenota_intro` → `body_content` + disattivazione `group_prenota_appuntamento_v1` legacy) · P11 contatti (token alignment + select aree dinamico CPT competenza) · Wave 5.1 Image Expansion (foto reali via Media Library, swap Picsum) · Wave 5 STEP 5 phantom resolution (~460 defer typography catalogati) · single-post JSX request Design (alta priorità SEO, 326 post) · cleanup 15 branch feat/chore orfani · EDITOR-HANDOFF v6.1 refresh
...
| **Wave 6.0 partial — CPT competenza content migration + 2 incident WSOD + sanitize chirurgico** (~3 ore totali wall-clock, 3 round). **Round 1 — Migration (v1.3.22)**: commit `9b5221c` merge feat/cpt-competenza-content-migration: template patch `single-competenza.php:213` `wp_kses_post($body_ext)` → `apply_filters('the_content', $body_ext)` per allineamento semantic post_content (wpautop + shortcode + oEmbed) + script `scripts/migrate-cpt-competenza-content.php` (231 righe, WP-CLI eval-file, dry-run/wet-run mode, idempotent, backup `_legacy_post_content_backup` postmeta) + docs/SCF_ORPHAN_FIELDS.md updated. Wet-run script su staging: **16 MIGRATED** (post_content → body_extended), **3 SKIP tier-1 already-populated** (2664/2665/2666 Elena-written pre-migration), **5 SKIP draft not-publish** (2680/2681/2705/2706/2707). Total: 24 CPT scanned. Audit integrity check PASS (body == backup byte-per-byte sui 16 migrated). Bonus: Code patched script args parsing per WP-CLI `eval-file` quirks (accept positional + `--wet-run`/`wet-run`/`wet`). **Round 2 — WSOD frontend rollback template (v1.3.23)**: dopo deploy v1.3.22, frontend competenze restituiva pagina bianca. Causa probabile: `apply_filters('the_content', $body_ext)` su content legacy migrato → fatal error (shortcode legacy non più registrati OR do_blocks parse fail OR oEmbed timeout sync HTTP). Commit `d93b50a` fix(emergency): rollback `single-competenza.php:213` → `echo wp_kses_post($body_ext)` (semantica originale Wave C). Migration DATI restano valide: HTML literal h2/p/strong renderizza correttamente con `wp_kses_post`. **Round 3 — WSOD admin sanitize chirurgico (v1.3.24)**: post-rollback template, frontend OK ma admin edit post 2670 (responsabilita-medica, migrato) ancora bianco mentre 2664 (tributario, Elena-written) OK. Differenza: body_extended di 2670 = raw copy post_content via `update_post_meta()` script SENZA sanitize; body_extended di 2664 = scritto via SCF wysiwyg ACF moderno + WP escape standard. Ipotesi: TinyMCE admin SCF wysiwyg crash su raw legacy HTML (encoding/control-chars/script/iframe/event-handlers). **Sanitize chirurgico via WP-CLI DB ops** (NO commit codice, solo dati staging): per i 16 migrated → strip control chars `[\\x00-\\x08\\x0B\\x0C\\x0E-\\x1F\\x7F]` + normalize line endings `\\r\\n|\\r` → `\\n` + `wp_kses_post()` (strip script/iframe/object/embed/form/event-handlers, preserve markup semantico). Test su 2670 → admin sblocco confermato. Apply su tutti 16 → tutti admin-editable. Frontend invariato post-sanitize (`wp_kses_post` idempotent su markup già pulito). Wave 6.0 partial CHIUSA. Wave 6.0 full (disable Gutenberg per CPT competenza + pre-sanitize pipeline migration v2) resta backlog post-cut.) | **1.3.24-wave-6-0-partial-stabilized** | **✅** |
...
### Lesson learned — Wave 6.0 partial CPT data migration: 3 takeaways critici (2026-05-13)

Migration `post_content` → `body_extended` postmeta su CPT competenza ha rivelato 3 pattern hard:

**1. `apply_filters('the_content', $field_value)` su content legacy compilato pre-Gutenberg può causare WSOD frontend** — la filter chain include `do_blocks` + `do_shortcode` + oEmbed handlers che possono fail su HTML literal classic (no `<!-- wp:* -->` markers) + shortcode legacy non più registrati + URL embed con timeout sync HTTP. Mitigazione: usare `wp_kses_post()` come safer default per content migrato; oppure pre-sanitize pipeline pre-call (strip non-registered shortcode + skip do_blocks per legacy + oEmbed disable temp).

**2. `update_post_meta(ID, 'body_extended', $raw_html)` su SCF wysiwyg field può crashare admin metabox TinyMCE** quando raw HTML contiene encoding doppio UTF-8 / control chars (`\\x00-\\x08\\x0B\\x0C\\x0E-\\x1F\\x7F`) / script/iframe/object/embed tag / inline event handlers (`on*=`). TinyMCE moderno sanitize input ma può fatal su content patologico. Mandatory pre-storage: `wp_kses_post()` + control chars strip + `\\r\\n|\\r` → `\\n` normalize.

**3. Migration data ops vs codice ops separate**: data migration via WP-CLI eval (`update_post_meta` + `wp_update_post`) modifica DB staging direct — NO commit codice associato. Quando emerge bug post-migration (es. WSOD), rollback è separato: data via `delete_post_meta` + restore from backup postmeta vs codice via `git revert`. Backup `_legacy_post_content_backup` postmeta è MANDATORY per rollback per-post idempotent senza re-deploy codice.
...
---
*Last updated: 2026-05-13 · v1.3.24-wave-6-0-partial-stabilized CUT-READY · Wave Elena Feedback Batch 1+2+3 + Wave 6.0 partial COMPLETATE: 18 fix Elena-approved totali (Batch 1: 13 Q+S · Batch 2: 3 M+H+C · Batch 3: 1 J · Wave 6.0 partial: #23 unblocking via data migration + sanitize chirurgico CPT competenza) + 2 chore (housekeeping v1.3.20: CSS 71 rule .sl-tier1__* + docs reorg · followups v1.3.21: #18 leftover helper + EDITOR-HANDOFF Page 2714 nota) via multi-agentic parallel (7 Code agent + 2 Explore audit + 2 incident WSOD gestiti via rollback + sanitize). Sequenza Design Handoff COMPLETATA 12/12 (storico). Elena ora completa minor UX fix via WP-Admin pre-cut. Handoff Elena pre-cut: `docs/HANDOFF_ELENA_PRE_CUT.md`. Lesson learned Wave 6.0 partial aggiunta a Workflow rules. Backlog post-cut: cleanup 15 branch feat/chore orfani + P11 contatti + #15 term consistency Elena + Wave 5 STEP 5 phantom resolution + Wave 5.1 Image Expansion + Wave 6.0 FULL (disable Gutenberg CPT competenza + pre-sanitize pipeline migration v2) + Wave 6.1 SCF orphan field cleanup + single-post JSX request Design + EDITOR-HANDOFF v6.1 refresh*
