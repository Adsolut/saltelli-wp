...
## Current state — v1.3.22-wave-6-0-partial-cpt-content-migration (CUT-READY)

**Last updated:** 2026-05-12 (Wave 6.0 partial pre-cut — CPT competenza content migration sblocco Elena edit confusion: template patch single-competenza.php body_extended render via `apply_filters('the_content', ...)` invece di `wp_kses_post()` per allineamento semantic con post_content (wpautop + shortcode + oEmbed) + script `scripts/migrate-cpt-competenza-content.php` (WP-CLI eval-file, dry-run/wet-run mode, idempotent, backup `_legacy_post_content_backup` postmeta, guard skip Gutenberg moderni / body_extended already / post_content empty) ready-to-execute su staging post-rsync · sblocca Elena su CPT competenza con post_content classic HTML compilato es. 2670 responsabilita-medica · Wave 6.0 full backlog post-cut (disable Gutenberg per CPT competenza).)
**Branch:** `main` · last merge `9b5221c` (Wave 6.0 partial CPT migration v1.3.22) · prev merge `17930e8` (chore batch3 follow-ups v1.3.21) · prev merge `66231b7` (chore housekeeping post-batch3 v1.3.20) · ~44 commit totali post-v1.3.13
...
---
*Last updated: 2026-05-12 · v1.3.21-chore-batch3-followups CUT-READY · Wave Elena Feedback Batch 1+2+3 + 2 chore COMPLETATE: 17 fix Elena-approved totali (Batch 1: 13 Q+S · Batch 2: 3 M+H+C · Batch 3: 1 J) + 2 chore (housekeeping v1.3.20: CSS 71 rule .sl-tier1__* + docs reorg · followups v1.3.21: #18 leftover helper + EDITOR-HANDOFF Page 2714 nota) via multi-agentic parallel (6 Code agent + 2 Explore audit). Sequenza Design Handoff COMPLETATA 12/12 (storico). Elena ora completa minor UX fix via WP-Admin pre-cut. Handoff Elena documento pre-cut: `docs/HANDOFF_ELENA_PRE_CUT.md`. Backlog post-cut: P11 contatti + #15 term consistency Elena + Wave 5 STEP 5 phantom resolution + Wave 5.1 Image Expansion + Wave 6.0 CPT competenza migration + Wave 6.1 SCF orphan field cleanup + single-post JSX request Design*
